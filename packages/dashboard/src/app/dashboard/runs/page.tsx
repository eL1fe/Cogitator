...
import { Play, Search, CheckCircle, XCircle, Clock, Loader2, Wifi, WifiOff } from 'lucide-react';
    ...
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between animate-fade-in">
        <div>
          ...
          <p className="text-text-secondary mt-1">View execution history and details</p>
        </div>
        ...
        {loading ? (
          ...
          <div className="py-12 text-center">
            ...
            <p className="text-text-secondary">
              {search || statusFilter ? 'Try different filters' : 'Start an agent to see runs here'}
            </p>
          </div>
        ) : (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      {run.model && <span className="text-sm text-text-muted">{run.model}</span>}
                    </div>
                    <p className="text-sm text-text-secondary truncate">
                      {run.input.slice(0, 100)}
                      {run.input.length > 100 ? '...' : ''}
                    </p>
                    <p className="text-xs text-text-muted mt-1">{run.id}</p>
                  </div>
                  ...
                  <div className="text-right text-xs text-text-muted w-24">
                    ...
                    {run.duration && (
                      <p className="text-text-secondary">{formatDuration(run.duration)}</p>
                    )}
                  </div>
                </Link>
              );
